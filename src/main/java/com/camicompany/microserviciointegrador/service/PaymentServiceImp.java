...
public class PaymentServiceImp implements PaymentService {
    ...
    public PaymentResponse createPayment(CreatePaymentRequest request) {
        ...
        try {
            ...
            log.info("Duplicate referencia_externa detected: {}",
                    referenciaExternaNormalized);
        ...
        try {
            HelipagosCreatePaymentRequest helipagosRequest = PaymentMapper.toHelipagosRequest(
                            request,
                            redirectUrl,
                            webhookUrl
                    );
            ...
            Payment updated = updatePaymentWithHelipagosResponse(
    ...
    @Transactional
    private Payment createPendingPayment(CreatePaymentRequest request, String referenciaExternaNormalized
    ) {
    ...
    private Payment updatePaymentWithHelipagosResponse(
        ...
        PaymentMapper.toEntityFromHelipagosResponse(
                payment, response);
        ...
        if (payment == null) {
            ...
            try
                ...
                log.info("Payment reconciled from webhook for id_sp {}", request.id_sp());
                    ...
                    log.error("Failed to reconcile payment from webhook for id_sp {}: {}",
                            request.id_sp(), e.getMessage());
                    return;
            }
        }
    ...
    private Payment tryReconcilePaymentFromWebhook(HelipagosWebhookRequest request){
        String referenciaExternaNormalized = PaymentMapper.normalizeReference(
                request.referencia_externa());

        Payment payment = paymentRepository
                .findByReferenciaExterna(referenciaExternaNormalized)
                .orElseThrow(()->
                        new ResourceNotFoundException("Payment not found with referencia_externa: "
                                + referenciaExternaNormalized));
        log.info("Reconciling payment {} from webhook referencia_externa {}",
                payment.getId(), referenciaExternaNormalized);
