...
public class PaymentSecurityIntegrationTest {
  ...
  void createPaymentShouldReturn201WithValidApiKey() throws Exception {
    ...
    when(helipagosClient.createPayment(any()))
        .thenReturn(
            new HelipagosCreatePaymentResponse(123, "GENERADA", "REF123", "http://checkout"));

    CreatePaymentRequest req =
        new CreatePaymentRequest(10000L, "Pago test", LocalDate.now().plusDays(1), "REF123");
  ...
  void getPaymentShouldReturn200WithValidApiKey() throws Exception {
    ...
    when(helipagosClient.getPayment(any()))
        .thenReturn(new HelipagosGetPaymentResponse(300, "GENERADA", "REF200"));

    mockMvc
        .perform(get(BASE_URL + "/" + payment.getIdSp()).header("x-api-key", apiKey))
  ...
  void getPaymentShouldReturn401WhenApiKeyIsInvalid() throws Exception {
    ...
    mockMvc
        .perform(get(BASE_URL + "/200").header("x-api-key", "invalid-api-key"))
        .andExpect(status().isUnauthorized());
  }
  ...
  void cancelPaymentShouldReturn200WithValidApiKey() throws Exception {
    ...
    when(helipagosClient.getPayment(any()))
        .thenReturn(new HelipagosGetPaymentResponse(300, "GENERADA", "REF300"));

    mockMvc
        .perform(delete(BASE_URL + "/" + payment.getIdSp()).header("x-api-key", apiKey))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.id_sp").value("300"))
        .andExpect(jsonPath("$.referencia_externa").value("REF300"))
  ...
  void cancelPaymentShouldReturn401WhenApiKeyIsInvalid() throws Exception {
    ...
    mockMvc
        .perform(delete(BASE_URL + "/300").header("x-api-key", "invalid-api-key"))
        .andExpect(status().isUnauthorized());
  }
