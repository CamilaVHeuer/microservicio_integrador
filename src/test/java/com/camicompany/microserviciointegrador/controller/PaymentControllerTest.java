...
public class PaymentControllerTest {
  ...
  @MockitoBean private ApiKeyFilter apiKeyFilter;
  ...
  void createPaymentShouldReturn201() throws Exception {
    CreatePaymentRequest req =
        new CreatePaymentRequest(
            10000L, "Pago de servicios", LocalDate.now().plusDays(1), "REF123456");
    PaymentResponse resp =
        new PaymentResponse(1L, "1", "REF123456", "GENERATED", "GENERADA", "url");
  ...
  void createPaymentWithInvalidReferenciaExternaShouldReturn400() throws Exception {
    CreatePaymentRequest req =
        new CreatePaymentRequest(10000L, "Pago de servicios", LocalDate.now().plusDays(1), "");
    mockMvc
        .perform(
            post(BASE_URL)
                .contentType(MediaType.APPLICATION_JSON)
        ...
        .andExpect(jsonPath("$.error").value("BAD_REQUEST"))
        .andExpect(
            jsonPath("$.message").value(containsString("The external reference cannot be blank")))
        .andExpect(jsonPath("$.timestamp").exists());
  }
  ...
  void createPaymentWithInvalidDescripcionShouldReturn400() throws Exception {
    CreatePaymentRequest req =
        new CreatePaymentRequest(10000L, "", LocalDate.now().plusDays(1), "REF123456");
    mockMvc
        .perform(
  ...
  void createPaymentWithInvalidAmountShouldReturn400() throws Exception {
    CreatePaymentRequest req =
        new CreatePaymentRequest(
            null, "Pago de servicios", LocalDate.now().plusDays(1), "REF123456");
    mockMvc
        .perform(
            post(BASE_URL)
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void createPaymentWithInvalidFechaVtoShouldReturn400() throws Exception {
    CreatePaymentRequest req =
        new CreatePaymentRequest(10000L, "Pago de servicios", null, "REF123456");
    mockMvc
        .perform(
            post(BASE_URL)
                .contentType(MediaType.APPLICATION_JSON)
        ...
        .andExpect(jsonPath("$.error").value("BAD_REQUEST"))
        .andExpect(
            jsonPath("$.message").value(containsString("The expiration date cannot be null")))
        .andExpect(jsonPath("$.timestamp").exists());
  }
  ...
  void createPaymentWithFechaVtoInPastShouldReturn400() throws Exception {
    CreatePaymentRequest req =
        new CreatePaymentRequest(
            10000L, "Pago de servicios", LocalDate.now().minusDays(1), "REF123456");
    mockMvc
        .perform(
            post(BASE_URL)
                .contentType(MediaType.APPLICATION_JSON)
        ...
        .andExpect(
            jsonPath("$.message")
                .value(containsString("The expiration date cannot be earlier than today")))
        .andExpect(jsonPath("$.timestamp").exists());
  }
  ...
  void createPaymentWhenBadRequestToHelipagosShouldReturn400() throws Exception {
    CreatePaymentRequest req =
        new CreatePaymentRequest(
            10000L, "Pago de servicios", LocalDate.now().minusDays(1), "REF123456");
    when(paymentService.createPayment(req))
  ...
  void getPaymentNotFoundShouldReturn404() throws Exception {
    when(paymentService.getPayment("1"))
        .thenThrow(new ResourceNotFoundException("Payment not found with id_sp: 99"));
    mockMvc
        .perform(get(BASE_URL + "/1"))
  ...
  void cancelPaymentNotFoundShouldReturn404() throws Exception {
    when(paymentService.cancelPayment("1"))
        .thenThrow(new ResourceNotFoundException("Payment not found with id_sp: 99"));
    mockMvc
  ...
  void webhookShouldReturn200() throws Exception {
    HelipagosWebhookRequest req =
        new HelipagosWebhookRequest(1, "PROCESADA", "REF123", null, null, null);
    mockMvc
        .perform(
  ...
  void webhookShouldReturn400WhenApiKeyIsInvalid() throws Exception {
    HelipagosWebhookRequest req =
        new HelipagosWebhookRequest(1, "PROCESADA", "REF123", null, null, null);
    doThrow(new ExternalServiceBadRequestException("Invalid webhook api-key"))
        .when(paymentService)
