...
public class PaymentIntegrationTest {
  ...
  @MockitoBean private HelipagosClient helipagosClient;
  ...
  void createPaymentShouldReturn201AndPersist() throws Exception {
    when(helipagosClient.createPayment(any()))
        .thenReturn(
            new HelipagosCreatePaymentResponse(123, "GENERADA", "REF123", "http://checkout-test"));

    CreatePaymentRequest req =
        new CreatePaymentRequest(
            10000L, "Pago de servicios", LocalDate.now().plusDays(1), "REF123");
  ...
  void createPaymentWhenMissingFieldShouldReturn400() throws Exception {
    CreatePaymentRequest req =
        new CreatePaymentRequest(null, "desc", LocalDate.now().plusDays(1), "REF1000");
    mockMvc
        .perform(
            post(BASE_URL)
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void getPaymentShouldReturn200AndSyncStatus() throws Exception {
    ...
    when(helipagosClient.getPayment(any()))
        .thenReturn(new HelipagosGetPaymentResponse(200, "GENERADA", "REF2000"));

    mockMvc
        .perform(get(BASE_URL + "/" + payment.getIdSp()))
  ...
  void getPaymentNotFoundShouldReturn404() throws Exception {
    mockMvc
        ...
        .andExpect(
            jsonPath("$.message").value(containsString("Payment not found with id_sp: 999999")))
        .andExpect(jsonPath("$.timestamp").exists());
  }
  ...
  void cancelPaymentShouldReturn200AndUpdateStatus() throws Exception {
    ...
    mockMvc
        .perform(delete(BASE_URL + "/" + payment.getIdSp()))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.paymentId").exists())
        .andExpect(jsonPath("$.id_sp").value(payment.getIdSp()))
        .andExpect(jsonPath("$.referencia_externa").value("REF123"))
  ...
  void cancelPaymentNotFoundShouldReturn404() throws Exception {
    ...
    mockMvc
        .perform(delete(BASE_URL + "/999999"))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.error").value("NOT_FOUND"))
        .andExpect(
            jsonPath("$.message").value(containsString("Payment not found with id_sp: 999999")))
  ...
  void webhookShouldUpdatePaymentStatusAndShouldReturn200() throws Exception {
    ...
    HelipagosWebhookRequest req =
        new HelipagosWebhookRequest(300, "PROCESADA", "REF300", null, null, null);
    mockMvc
        .perform(
  ...
  void webhookShouldBeIdempotentAndReturn200() throws Exception {
    ...
    HelipagosWebhookRequest req =
        new HelipagosWebhookRequest(400, "PROCESADA", "REF400", null, null, null);
    ...
    // Assert final state
    Payment updated = paymentRepository.findByIdSp("400").orElseThrow();
  ...
  void webhookShouldReturn400WhenApiKeyIsMissing() throws Exception {
    HelipagosWebhookRequest req =
        new HelipagosWebhookRequest(301, "PROCESADA", "REF301", null, null, null);
    mockMvc
        .perform(
            post(BASE_URL + "/webhook")
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void webhookShouldReturn400WhenApiKeyIsInvalid() throws Exception {
    HelipagosWebhookRequest req =
        new HelipagosWebhookRequest(302, "PROCESADA", "REF302", null, null, null);
    mockMvc
        .perform(
            post(BASE_URL + "/webhook")
